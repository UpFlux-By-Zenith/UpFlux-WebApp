...
namespace UpFlux_WebService
{
    ...
    public class ControlChannelService : ControlChannel.ControlChannelBase, IControlChannelService
    {
            ...
            try
            {
                ...
                foreach (var deviceUuid in metadata.TargetDevices)
                    if (succeededDevices.Contains(deviceUuid))
                    {
                        ...
                        await UpdateApplicationAndNotify(applicationRepository, notificationService, application,
                            metadata, deviceUuid);
                    }
                    else
                    {
        ...
        private async Task UpdateApplicationAndNotify(IApplicationRepository repository,
            INotificationService notificationService, Application application, UpdateMetadata metadata,
            string deviceUuid)
        {
                ...
                foreach (var dv in resp.DeviceVersionsList)
                {
                    ...
                    if (dv.Available.Count > 0)
                    {
                        ...
                        if (versionsToAdd.Any())
                            ...
                            {
                                var newVersion = new MachineStoredVersions()
                                {
